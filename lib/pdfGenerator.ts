...
import { PredictionReport, Symptom } from '../types';

/**
 * Generates and downloads a professional PDF clinical summary for a symptom checker report.
 */
export function generatePDFReport(report: PredictionReport, userName: string, allSymptoms: Symptom[]) {
  // Create instance of jsPDF (A4 size, portrait)
  const doc = new jsPDF({
  ...
  // Header Design
  // Header background highlight top bar
  ...
  report.predictions.forEach((pred, index) => {
    ...
    // Draw a small visual confidence bar
    y += 3;
    ...
    // Key Recommendations (only for the top prediction to keep PDF neat)
    if (index === 0) {
      y += 6;
      doc.setFont('Helvetica', 'bold');
      doc.setFontSize(10);
  ...
  // Footer Disclaimer Block (Always stays at bottom)
